...
│
├── 00-overview/                                  # 0. Cổng vào — đọc trước khi dùng bất kỳ mục nào khác
│   ├── getting-started-checklist.md              # Checklist setup dự án mới từ A-Z bằng template này
│   ├── how-to-use-this-template.md               # Thứ tự đọc/áp dụng 01→05, ai dùng phần nào (lead/dev/DevOps)
│   └── approved-tech-stack.md                    # Tech radar: ngôn ngữ/framework/tool được duyệt dùng
│
...
│   ├── versioning-policy.md                      # Quy tắc semver — khi nào bump major/minor/patch
│   ├── coding-standards/                         # Config lint/format mẫu, enforce chuẩn bằng máy
│   │   ├── typescript/.eslintrc.json             # Cấu hình lint mẫu cho dự án TypeScript
│   │   ├── typescript/.prettierrc.json           # Cấu hình format code mẫu cho TypeScript
│   │   ├── python/ruff.toml                      # Cấu hình lint/format mẫu cho dự án Python
│   │   └── rust/rustfmt.toml                     # Cấu hình format code mẫu cho dự án Rust
│   └── testing/                                  # Chuẩn testing + template thực thi trong cùng 1 chỗ
│       ├── automated-testing-standards.md        # Tiêu chuẩn viết Unit, Integration và E2E Test
│       ├── unittest-template.md                  # Template chuẩn viết Unit test (FIRST + AAA)
│       └── e2e-testcase-template.md              # Template chuẩn viết E2E test case
│
...
│   ├── non-functional-requirements/              # Yêu cầu phi chức năng — tách riêng để không bị bỏ sót
│   │   ├── nfr-checklist.md                      # Checklist performance, scalability, availability...
│   │   └── threat-model-template.md              # Biểu mẫu phân tích rủi ro bảo mật khi thiết kế
...
│   ├── infrastructure-as-code/                   # Mẫu triển khai hạ tầng lên môi trường thật
│   │   ├── terraform-module-template/            # Module Terraform mẫu để dựng hạ tầng cloud
│   │   └── k8s-manifest-template/                # Manifest Kubernetes mẫu để deploy service
...
│   ├── incident-response/                        # Xử lý sự cố vận hành hàng ngày (khác thảm họa)
│   │   └── runbook-template.md                   # Kịch bản xử lý sự cố production thường gặp
...
├── CONTRIBUTING.md                               # Cách clone/áp dụng repo này cho một dự án mới
├── CODE_OF_CONDUCT.md                            # Quy tắc ứng xử chuẩn cộng đồng đóng góp (GitHub)
├── SECURITY.md                                   # Quy trình report và xử lý lỗ hổng bảo mật
